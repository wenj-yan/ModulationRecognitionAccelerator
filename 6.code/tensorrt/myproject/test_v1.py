...
# 生成engine
    ...
    # 绑定上下文
    context.set_input_shape("inputs", (1,128,2))
    print("binding successful!")

    start_time = time.time()
#xun huan
    for signal,class_i in zip(random_signals,random_class):
        signal = signal.reshape(1,1024,2)
        input_data =signal[:,:128,:]
    # 前处理
 ...
 # 分配内存缓冲区
    ...
    # 同步Cuda流
        stream.synchronize()
        i = np.argmax(output_buffer)
        #print(f"pred:{i},true:{class_i}")
        if i == class_i:
            accurary += 1
        else:
            pass
